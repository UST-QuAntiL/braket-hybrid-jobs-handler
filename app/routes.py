...
from flask import jsonify, abort, request, send_from_directory, make_response, url_for
import logging
import os
import string
import random

@app.route('/aws-runtime-handler/api/v1.0/generate-hybrid-program', methods=['POST'])
def generate_hybrid_program():
    """Put hybrid program generation job in queue. Return location of the later result."""
    app.logger.info('Received request for hybrid program generation2...')
    ...
    app.logger.info("Das ist die URL ")
    ...
    content_location = '/aws-runtime-handler/api/v1.0/results/' + result.id
    response = jsonify({'Location': content_location})
    response.status_code = 202
    response.headers['Location'] = content_location
    app.logger.info('Location...'+ content_location)
    app.logger.info('The response:')
    return response


@app.route('/aws-runtime-handler/api/v1.0/results/<result_id>', methods=['GET'])
def get_result(result_id):
    ...
    try:
        ...
        return make_response(jsonify({'error': 'Result ID ' + result_id + ' does not exist', 'statusCode': '404'}), 404)


@app.route('/aws-runtime-handler/api/v1.0/uploads/<name>')
def download_uploaded_file(name):
    return send_from_directory(app.config["UPLOAD_FOLDER"], name)


@app.route('/aws-runtime-handler/api/v1.0/hybrid-programs/<name>')
def download_generated_file(name):
    return send_from_directory(app.config["RESULT_FOLDER"], name)


@app.route('/aws-runtime-handler/api/v1.0/version', methods=['GET'])
def version():
